...
def main():
    ...
    def copy_file(input_file_name, file_name = 'LICENSE', type = 'w'):
        data = pkgutil.get_data('tokapy', input_file_name)
    ...
    def webpack_file(input_file_name, **types):
        ...
        for key, value in types.items():
                if key == '-p':
                    if '{{ path }}' in data:
                        data = data.replace('{{ path }}', value)
                if key == '-f':
                    if '{{ filename }}' in data:
                        data = data.replace('{{ filename }}', value)
                if key == '-e':
                    if '{{ entry }}' in data:
                        data = data.replace('{{ entry }}', value)
                output_file.write(data)
    ...
    if args_len == 1 or (args_len == 2 and args[1] == '-v'):
        ...
        if args_len < 4:
            ...
            if args3 in webpack_orders:
                types = {args3:args4}
                webpack_file('lib/webpack/config.js', **types)
            else:
                webpack_warning()
        elif args_len == 6:
            args3 = args[2]
        ...
        if args[1] == 'license' or args[1] == 'l':
            ...
            if args2 == '-d':
                ...
                copy_file('lib/gitignore/Default.gitignore', '.gitignore')
                find = True
            else:
                ...
                for lang in lang_list:
                    for i in langs:
                        if lang.lower() == i.lower():
                            ...
                            copy_file('lib/gitignore/' + i + '.gitignore', '.gitignore')
                            find = True
            if not find:
                gitignore_warning()
        ...
        if args[1] == '.gitignore' or args[1] == 'g' or args[1] == 'git':
            ...
            if args2 == '-d':
                print('The .gitignore file for your default project is saved!')
                if args3 == '-a':
                    copy_file('lib/gitignore/Default.gitignore', '.gitignore', 'a')
                else:
                    copy_file('lib/gitignore/Default.gitignore', '.gitignore')
                find = True
                ...
                for lang in lang_list:
                    for i in langs:
                        if lang.lower() == i.lower():
                            print('The .gitignore file for your ' + i.lower() + ' project is saved!')
                            if args3 == '-a':
                                copy_file('lib/gitignore/' + i + '.gitignore', '.gitignore', 'a')
                            else:
                                copy_file('lib/gitignore/' + i + '.gitignore', '.gitignore')
                            find = True
            if not find:
